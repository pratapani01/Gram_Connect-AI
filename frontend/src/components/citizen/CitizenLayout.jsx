  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="p-6 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-primary-600 font-medium">Citizen Portal</div>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 mx-3 mt-3 bg-primary-50 rounded-xl">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-primary-200 flex items-center justify-center overflow-hidden flex-shrink-0">
            {user?.profilePicture
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-500 truncate">{user?.village?.name || 'Village not set'}</p>
          </div>
        </div>
      </div>
...
<div className="flex-1 text-center lg:text-left">
  ...
  <h2 className="text-gray-500 text-sm hidden lg:block truncate">
    📍 {user?.village?.name}, {user?.district?.name}, {user?.state?.name}
          ...
          <div className="flex items-center gap-3">
            <NavLink to="/citizen/notifications" className="relative p-2 text-gray-500 hover:text-primary-600">
            ...
            <NavLink to="/citizen/profile">
              <div className="w-9 h-9 rounded-full bg-primary-200 flex items-center justify-center overflow-hidden">
                {user?.profilePicture
                  ? <img src={user.profilePicture} alt="" className="w-full h-full object-cover" />
                  : <span className="text-primary-700 font-bold text-sm">{user?.name?.[0]?.toUpperCase()}</span>
                }
